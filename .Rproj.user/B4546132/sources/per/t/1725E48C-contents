...
CytoTRACE_mod <- function(mat, batch = NULL, enableFast = TRUE,
  ...
  pqgenes <- is.na(Matrix::rowSums(mat>0)) | scUtils::rowVars_spm(mat) == 0
  ...
  batches <- parallel::mclapply(subsamples, mc.cores = min(chunk, ncores), function(subsample){
    ...
    mat <- Matrix::t(Matrix::t(mat)/apply(mat, 2, sum))*1000000
